...
if(!file.exists('app/www/federal_critical_habitat_just_BC.gpkg')){
  ...
  crit_hab_defs_mp = sf::st_make_valid(crit_hab_defs_mp)
...
if(!file.exists('app/www/DFO_SARA_occ_data_QGIS_very_simplified.gpkg')){
  ...
  dfo_sara_in_bc = read_sf(paste0(onedrive_path,"dfo_occurrences_in_BC_all_species.gpkg"))

  # Make sure bull trout records within the giant polygon are removed.
  dfo_sara_in_bc_bulltrout = dfo_sara_in_bc |>
    dplyr::filter(Common_Name_EN=='Bull Trout')

  bulltrout_polygon = dfo_sara_in_bc_bulltrout |> dplyr::arrange(dplyr::desc(Shape_Area)) |> dplyr::slice(1)

  bulltrout_in_polygon = as.numeric(
    dfo_sara_in_bc_bulltrout |>
    sf::st_intersects(bulltrout_polygon)
  )

  # ggplot() +
  #   geom_sf(data = bulltrout_polygon) +
  #   geom_sf(data = dfo_sara_in_bc_bulltrout[is.na(bulltrout_in_polygon),])
  dfo_sara_in_bc_bulltrout = dplyr::bind_rows(bulltrout_polygon,dfo_sara_in_bc_bulltrout[is.na(bulltrout_in_polygon),])

  dfo_sara_in_bc = dfo_sara_in_bc |>
    dplyr::filter(Common_Name_EN != 'Bull Trout') |>
    dplyr::bind_rows(dfo_sara_in_bc_bulltrout)
  ...
  # Remove little tidbits of bulltrout north of the giant polygon.

  # Update CRS
  dfo_sara_in_bc = sf::st_transform(dfo_sara_in_bc, 4326)

  # Only retain species that aren't marine
  dfo_sara_in_bc = dfo_sara_in_bc |>
    dplyr::filter(Eco_Type != 'Marine')

  # Ensure we are just taking polygons and multipolygons.
  # dfo_sara_in_bc = dfo_sara_in_bc |>
  #   dplyr::filter(sf::st_geometry_type(geom) %in% c("MULTIPOLYGON","POLYGON"))

  # ggplot() + geom_sf(data = dfo_sara_in_bc)
  #
...
#     n_empty = 0L)
  ...
  # Simplify geometries a bit.
  dfo_sara_in_bc = sf::st_make_valid(dfo_sara_in_bc)
  dfo_sara_in_bc_s = sf::st_simplify(dfo_sara_in_bc, dTolerance = 50)

  # Recombine the polygons and the random geometries subsets.
  # dfo_sara_in_bc_s = dplyr::bind_rows(dfo_sara_in_bc_polys,dfo_sara_in_bc_lines)
  ...
  # dfo_sara_in_bc_s_multip = sf::st_cast(dfo_sara_in_bc_s, "MULTIPOLYGON")

  # ggplot() + geom_sf(data = dfo_sara_in_bc_s_multip)
  # object.size(dfo_sara_in_bc_v_s)

  # sf::write_sf(dfo_sara_in_bc, 'app/www/DFO_SARA_species_occurrences.gpkg')
  sf::write_sf(dfo_sara_in_bc_s, 'data/DFO_SARA_occ_data_QGIS_simplified.gpkg')
  sf::write_sf(dfo_sara_in_bc_s, 'app/www/DFO_SARA_occ_data_QGIS_very_simplified.gpkg')
...
# Further simplification of spatial files.
...
# dfo_sara_occ_dat_v_s = sf::st_simplify(dfo_sara_occ_dat, dTolerance = 100)
...
#   addTiles() |>
#   addPolylines(
#     data = dfo_sara_in_bc_s |> st_cast("MULTIPOLYGON")
#   )
...
for(i in 1:length(boundary_files)){
  ...
  if(!boundary_col_name %in% names(sp_no_insects)){
